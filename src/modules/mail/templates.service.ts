...
export class EmailTemplatesService {
    ...
    constructor(private readonly configService: ConfigService) {
        ...
        this.dashboardUrl = this.configService.get<string>('DASHBOARD_URL') || 'http://localhost:3000/dashboard';
        this.resetUrlBase = this.configService.get<string>('RESET_URL') || 'http://localhost:3000/reset-password';
    }
    ...
    renderPasswordResetEmail(data: { name: string; email: string; code: string; expiryMinutes: number }): string {
        const template = fs.readFileSync(
